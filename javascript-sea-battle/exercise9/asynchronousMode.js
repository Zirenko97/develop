...
class App {
    ...
    shipArrangement(player, shipCount, maxShipLength) {  // создание и размещение кораблей
        ...
        let length = Math.floor(Math.random() * maxShipLength) + 1;
        alert(`Длина корабля ${shipName} в составе эскадры AIPlayer: ${length}`);
        let Placed = false;

        while(!Placed) {
            Placed = player.placeShips(shipName, length, 0, {x: 0, y: 0});
        }
    ...
    async run() {  // весь игровой процесс
        let varOpponent;
        while(varOpponent !== 1 || varOpponent !== 0) {
        varOpponent = parseInt(prompt("С кем сыграть то хочешь? 1 - человек, 0 - компьютер"));
        if(varOpponent === 1) {
        ...
        this.firstPlayer = new Player(prompt("имя первого игрока :"), this._boardSize);
        alert("Против тебя сразится AIPlayer");
        this.secondPlayer = new AIPlayer("AIPlayer", this._boardSize);
        break;
    } else {
...
app.run().catch(err => console.log("ошибочка: ", err));
