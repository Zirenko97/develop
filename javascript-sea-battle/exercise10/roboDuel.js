...
class Board {
    constructor(size) {
        ...
        for(let i = 0; i < size; i++) {
            ...
            for(let j = 0; j < size; j++) {
                this._grid[i].push(null);    // инициализация двумерного поля null 
            }
        }
        this._ships = [];                    // массив кораблей на поле [0] - ship1, [1] - ship2, [2] - ship3...
    }
    get size() {
        return this._size;
    }
    ...
    if(ship.orientation === 0) {                    // если горизонтальный корабль
        if(y + ship.length > this._size || y < 0) {
        ...
        if(x + ship.length > this._size || x < 0) {
            alert("вышел за рамки");
            return false;
        }
        for(let i = 0; i < ship.length; i++) 
    ...
    this._ships.push(ship);                         // в массив кораблей добавляется этот корабль
    ship.startPosition = {x, y};                    // в свойства корабля - начальная позиция добавляются координаты
    return true;
}

    findAvailableCells() {                          // возврат пустых доступных клеток в виде координат x, y
        let cells =[];
...
class Player {
   ...
   async takeTurn(opponent) {                                   // возврат экземпляра оппонента по введенным координатам
...
class App {
    ...
    sleep(ms) {
        return new Promise((resolve) => setTimeout(resolve, ms));                // промис для таймаута в 1 сек
    }

    shipArrangement(player, shipCount, maxShipLength) {                          // создание и размещение кораблей

        if(player instanceof AIPlayer) {                                         // если класс игрока - ИИ, корабли расставляются случайным образом
        for(let i = 0; i < shipCount; i++) {
        let shipName = `Ship ${i + 1}`;
        let length = Math.floor(Math.random() * maxShipLength) + 1;               // random() возвращает дробное число от 0 до 1 не включая саму единицу
        alert(`Длина корабля ${shipName} в составе эскадры AIPlayer: ${length}`); // оповещение о длине корабля ИИ
        let Placed = false;

        while(!Placed) {                                                          // цикл будет пытаться разместить пока не разместит
            Placed = player.placeShips(shipName, length, 0, {x: 0, y: 0});        // ориентация и координаты на входе будут проигнорированы и переопределены на выходе
            }                                                                     // поэтому вводим любые числа
        }
        alert("AIPlayer разместил свои корабли!");
        return;
    }
        ...
        for(let i = 0; i < shipCount; i++) {                                      // цикл проходит по всем кораблям
            ...
            while(!Placed) {
                ...
                if(!shipName) {
                    shipName = `Ship ${i + 1}`;                                   // если корабль не назван задаем стандартное имя
                }
                let shipLength = null;
                let isVertical = null;
    ...
    async run() {  // весь игровой процесс

       let statusOpponents = false;
       while(!statusOpponents) {
        let varOpponent1 = parseInt(prompt("кто биться будет? 1 - человек, 0 - компьютер"));
        if(varOpponent1 === 1) {
        this.firstPlayer = new HumanPlayer(prompt("имя первого игрока :"), this._boardSize);
    } else if (varOpponent1 === 0) {
        this.firstPlayer = new AIPlayer("AIMax", this._boardSize);
        alert("AIMax будут тебя звать");
    } else {
           alert("Введи только 1 или 0");
           continue;
        } 

        let varOpponent2 = parseInt(prompt("кто будет второй игрок? 1 - человек, 0 - компьютер"));
        if(varOpponent2 === 1) {
        this.secondPlayer = new HumanPlayer(prompt("имя второго игрока :"), this._boardSize);
        statusOpponents = true;
        break;
    } else if (varOpponent2 === 0) {
        this.secondPlayer = new AIPlayer("AIAlex", this._boardSize);
        alert("AIAlex будут тебя звать");
        statusOpponents = true;
        break;
    } else {
           alert("Введи только 1 или 0");
    ...
    let length = null;
    while(length == null) {
        length = parseInt(prompt("введи максимальную длину кораблей"));
    ...
    alert("игра началась!");
     ...
     while(true) {
        await this.sleep(1000);  // асинхронная функция задержки
        let isHit;
        let res = null;
        let coordAttack;
        ...
        while(true) {
        coordAttack = await currentPlayer.takeTurn(otherPlayer);
        if(coordAttack.x < 0 || coordAttack.x >= otherPlayer.boardSize || coordAttack.y < 0 || coordAttack.y >= otherPlayer.boardSize) {
            alert("куда бъешь то!?");
            continue;
            }
            const cell = otherPlayer.board.grid[coordAttack.x][coordAttack.y];   // клетка куда ударили проверяется до удара по ней
            if(cell === 'miss' || cell?.ship?.hits[cell.index]) {                // ?. - если объекта нет, вернет не ошибку, а тихий undefind
                alert("ты сюда уже бил");
                continue;
            }
        isHit = otherPlayer.board.receiveAttack(coordAttack.x, coordAttack.y);   // координаты валидны, производится атака по ним
        console.log(`"${currentPlayer.name} (${coordAttack.x}, ${coordAttack.y})"`);
        break;
        }
        ...
        const cell = otherPlayer.board.grid[coordAttack.x][coordAttack.y];
            if(cell?.ship?.isSunk()) {
                ...
                res = otherPlayer.board.ships.every(ship => ship.isSunk()); // после попадания и проверки на затопление корабля
                if(res) {                                                   // проверка не затоплены ли все корабли
            ...
            let tempPlayer = currentPlayer;  // смена игрока
...
class HumanPlayer extends Player {
    constructor(name, boardSize) {
        super(name, boardSize);                                                // доступ к родительскому классу super
    }                                                                          // геттеры и сеттеры в доступе так же с родительского класс
    placeShips(shipName, length, isVertical, startPosition) {                  // создание корабля и расположение его на поле
...
class AIPlayer extends Player {
    ...
    placeShips(shipName, length, isVertical, startPosition) {                   // создание корабля и расположение его на поле
       let placed = false;                                                      // ориентация и координаты на входе игнорируются так как внутри задаются рандомно
       while(!placed) {
...
app.run().catch(err => console.log("ошибочка: ", err));                         // ловим ошибку асинхронного кода методом промиса .catch
