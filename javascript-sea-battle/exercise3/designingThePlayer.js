...
class Player {
    ...
    takeTurn(opponent) {
        let x = prompt("введи x координаты удара");
        let y = prompt("введи y координаты удара");
        return {
            x: x,
            y: y,
            opponent: opponent
...
const name = prompt("Твое имя");
const boardSize = prompt("ширина поля игрока:");
alert(name + " " + boardSize);
const play = new Player(name, boardSize);

console.log('"' + play.name + '", ' + play.board.size);
